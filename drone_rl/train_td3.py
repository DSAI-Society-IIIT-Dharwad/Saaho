...
def main():
    ...
    if os.path.exists("model_td3.pt"):
        agent.load("model_td3.pt")
        print("Resuming from saved TD3 model 🔄")

    total_steps = 0
    ep_rewards  = []

    print(f"\n{'='*55}")
    print(f" TurtleBot3 TD3 Training  |  {NUM_EPISODES} episodes")
    print(f"{'='*55}\n")
        ...
        for ep in range(1, NUM_EPISODES + 1):
            ...
            while not done and rclpy.ok():
                ...
                if total_steps < WARM_UP_STEPS:
                    ...
                    import numpy as np
                    action = np.array([
                        np.random.uniform(0.0, 0.22),    # linear
                        np.random.uniform(-2.0, 2.0),    # angular
                    ], dtype=np.float32)
                else:
            ...
            if ep % SAVE_EVERY == 0:
                agent.save(f"model_td3_ep{ep}.pt")
                agent.save("model_td3.pt")

    except (KeyboardInterrupt, ExternalShutdownException):
        print("\n[Interrupt] Shutting down...")
    finally:
        # ── final save ─────────────────────────────────────────────────────────────
        agent.save("model_td3_interrupted.pt")
        print("TD3 model saved on shutdown.")
